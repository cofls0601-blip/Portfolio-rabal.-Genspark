...
MOBILE_CSS = """
<style>
:root { --app-font: -apple-system, BlinkMacSystemFont, 'Segoe UI', 'Noto Sans KR', sans-serif; }
...
# ---------- 전략 스펙 (외부 설정 파일) ----------
# 전략 정의·자산 유니버스·리밸런싱 규칙·파라미터는 코드가 아니라 strategy_spec.json 에 있다.
...
def get_specs():
    """현재 스펙 파일의 {code: spec} 사전 — 호출마다 최신 파일을 읽는다."""
    try:
        return se.load_specs(SPEC_PATH)
    except Exception:
        return se.specs_from_text(se.DEFAULT_SPECS)

# 스펙에서 기본 전략 설정(KV 'strategies' 시드용)·우선순위·전용 규칙 코드 집합을 파생한다
DEFAULT_STRATEGIES = se.specs_to_configs(get_specs())
STRATEGY_DISPLAY_ORDER = [s['code'] for s in sorted(get_specs().values(), key=lambda x: x.get('display_order', 99))]
KNOWN_STRATEGIES = {c for c, s in get_specs().items() if s.get('rule') != 'static'}  # 전용 규칙이 정의된 전략(이름 변경 불가)

def default_assets():
    rows = []
    i = 0
    for spec in sorted(get_specs().values(), key=lambda x: x.get('display_order', 99)):
        for a in spec.get('assets', []):
            ticker = a.get('ticker', '')
            rows.append({
                'id': str(i), 'strategy': spec['code'], 'ticker': ticker, 'name': a.get('name', ''),
                'market': a.get('market', 'KR'), 'role': a.get('role', ''), 'target_pct': float(a.get('target_pct', 0.0)),
                'shares': 0.0, 'close': (1.0 if ticker == 'CASH' else 0.0), 'prices': [],
                'signal_ticker': a.get('signal_ticker') or ticker, 'category': a.get('category', '기타'),
...
def _state_default(k):
    defaults = {
        'assets': default_assets().to_dict('records'), 'history': [], 'equity': [],
        'cashflows': [], 'benchmarks': [], 'strategies': DEFAULT_STRATEGIES,
        'category_targets': {c: 0.0 for c in CATEGORY_OPTIONS}, 'executions': [],
        'account_cash': {}, 'price_policy': 'strict', 'price_mode': 'close',
        'custom_benchmarks': {},
    }
    return defaults.get(k)

def get_state(k):
    """SQLite KV를 방어적으로 읽는다. 손상 원문은 quarantine에 보존한다."""
    init_db()
    con = sqlite3.connect(DB_PATH)
    try:
        con.execute('CREATE TABLE IF NOT EXISTS kv_quarantine(k TEXT, raw TEXT, reason TEXT, created_at TEXT DEFAULT CURRENT_TIMESTAMP)')
        r = con.execute('SELECT v FROM kv WHERE k=?', (k,)).fetchone()
        if r is None:
            value = _state_default(k)
            con.execute('INSERT OR REPLACE INTO kv(k,v) VALUES(?,?)', (k, json.dumps(value, ensure_ascii=False, default=str)))
            con.commit()
            return value
        try:
            return json.loads(r[0])
        except (TypeError, ValueError, json.JSONDecodeError) as exc:
            con.execute('INSERT INTO kv_quarantine(k,raw,reason) VALUES(?,?,?)', (k, str(r[0]), f'JSON decode error: {exc}'))
            value = _state_default(k)
            con.execute('INSERT OR REPLACE INTO kv(k,v) VALUES(?,?)', (k, json.dumps(value, ensure_ascii=False, default=str)))
            con.commit()
            return value
...
def ordered_strategy_codes(cfgs=None, active_only=True):
    """Action Plan/대시보드에 스펙의 display_order 순, 그 다음 스펙에 없는 전략 순으로."""
    cfgs = cfgs if cfgs is not None else get_strategies()
    if active_only: cfgs = [c for c in cfgs if c.get('active', True)]
    codes = [c['code'] for c in cfgs]
    specs = get_specs()
    known = [s['code'] for s in sorted(specs.values(), key=lambda x: x.get('display_order', 99)) if s['code'] in codes]
...
def rename_strategy(old_code, new_code, assets_df):
    ...
    _specs_now = get_specs()
    _sp_old = _specs_now.get(old_code)
    if _sp_old is not None and _sp_old.get('rule') != 'static':
        return assets_df, False, f'{old_code}는 전용 리밸런싱 규칙({_sp_old.get("rule")})이 스펙에 정의되어 있어 이름을 바꿀 수 없습니다. 계좌 별명만 바꿔주세요.'
    ...
    put_state('strategies', cfgs)
    if _sp_old is not None:
        _specs_now[new_code] = _specs_now.pop(old_code)
        _specs_now[new_code]['code'] = new_code
        se.save_specs(SPEC_PATH, _specs_now)
...
def parse_ui_date(value, label='날짜'):
    text = str(value or '').strip()
    try:
        return pd.Timestamp(text).date()
    except Exception:
        st.error(f'{label} 형식이 올바르지 않습니다. YYYY-MM-DD로 입력하세요.')
        return None

def download_link(label, content, filename, mime='application/octet-stream'):
    raw = content.encode('utf-8') if isinstance(content, str) else bytes(content)
    b64 = base64.b64encode(raw).decode('ascii')
    href = f'data:{mime};base64,{b64}'
    safe_label = str(label).replace('&','&amp;').replace('<','&lt;').replace('>','&gt;').replace('"','&quot;')
    safe_name = str(filename).replace('&','&amp;').replace('<','&lt;').replace('>','&gt;').replace('"','&quot;')
    st.markdown(f'<a href="{href}" download="{safe_name}" style="display:inline-block;padding:.55rem .85rem;border:1px solid var(--border);border-radius:10px;text-decoration:none;font-weight:650;color:var(--text);background:var(--surface);">{safe_label}</a>', unsafe_allow_html=True)
...
def export_backup_dict(overrides=None):
    overrides = overrides or {}
    data = {k: overrides[k] if k in overrides else get_state(k) for k in ALL_KV_KEYS}
...
def _snapshot_payload(assets_df, run_date, plan_text=None):
    """저장/백업 미리보기가 같은 계산을 쓰도록 스냅샷 1건을 순수하게 만든다."""
    ...
    return rec, {'date': ds, 'value': grand}, grand


def backup_with_snapshot(assets_df, run_date, plan_text=None):
    """아직 저장 버튼을 누르기 전에도 '저장 후 상태'와 동일한 JSON을 만든다."""
    rec, eq_rec, _ = _snapshot_payload(assets_df, run_date, plan_text)
    ds = rec['date']
    h = [x for x in get_state('history') if x.get('date') != ds]
    h.insert(0, rec)
    e = [x for x in get_state('equity') if x.get('date') != ds]
    e.append(eq_rec)
    return export_backup_dict({'history': h, 'equity': e})


def delete_history_snapshot(ds):
    """해당 날짜의 스냅샷만 삭제한다. 보유자산·전략·체결 원장은 유지한다."""
    h = [x for x in get_state('history') if x.get('date') != ds]
    e = [x for x in get_state('equity') if x.get('date') != ds]
    put_state('history', h)
    put_state('equity', e)
...
def _nested_get(d, path, default=None):
    ...
    for pth in path:
        if not isinstance(cur, dict) or pth not in cur:
            return default
        cur = cur[pth]
    return cur
...
def render_friendly_rule_params(rule, params, key_prefix):
    ...
    result = json.loads(json.dumps(params or {}, ensure_ascii=False))
    schema = se.rule_ui_schema(rule) if hasattr(se, 'rule_ui_schema') else []
    if not schema:
        st.caption('이 규칙은 별도로 조정할 조건이 없습니다.')
        return result
    for i, field in enumerate(schema):
        path = field['path']; typ = field.get('type', 'text'); default = field.get('default')
        value = _nested_get(result, path, default)
        key = f'{key_prefix}_{i}_' + '_'.join(path)
        kw = {'key': key}
        if field.get('help'): kw['help'] = field['help']
        if typ == 'bool':
            out = st.checkbox(field['label'], value=bool(value), **kw)
        elif typ == 'int':
            out = st.number_input(field['label'], value=int(value), min_value=int(field.get('min', -1000000)), max_value=int(field.get('max', 1000000)), step=int(field.get('step', 1)), **kw)
            out = int(out)
        elif typ == 'float':
            out = st.number_input(field['label'], value=float(value), min_value=float(field.get('min', -1e9)), max_value=float(field.get('max', 1e9)), step=float(field.get('step', 1.0)), **kw)
            out = float(out)
        elif typ == 'fraction_pct':
            shown = float(value) * 100.0
            shown = st.number_input(field['label'], value=shown, min_value=float(field.get('min', -100.0)), max_value=float(field.get('max', 100.0)), step=float(field.get('step', 1.0)), **kw)
            out = float(shown) / 100.0
        elif typ == 'select':
            opts = list(field.get('options') or [])
            if value not in opts and opts: value = opts[0]
            out = st.selectbox(field['label'], opts, index=opts.index(value) if value in opts else 0, **kw)
        elif typ == 'csv_list':
            shown = ', '.join(str(x) for x in value) if isinstance(value, list) else str(value or '')
            raw = st.text_input(field['label'], value=shown, **kw)
            out = [x.strip() for x in raw.split(',') if x.strip()]
        else:
            out = st.text_input(field['label'], value=str(value or ''), **kw)
        _nested_set(result, path, out)
    return result
...
def save_history_snapshot(assets_df, run_date, plan_text=None):
    ...
    rec, eq_rec, grand = _snapshot_payload(assets_df, run_date, plan_text)
    ds = rec['date']
...
def ensure_cash_rows(assets_df):
    ...
    if not isinstance(legacy_cash, dict): legacy_cash = {}
    changed = False
    for cfg in cfgs:
        code = cfg['code']; sub = assets_df[assets_df['strategy'].eq(code)]
...
.stButton>button,.stButton>button{border-radius:10px !important; min-height:2.65rem; font-weight:650; border:1px solid var(--border); background:var(--surface); color:var(--text); transition:.15s ease;}
.stButton>button:hover,.stButton>button:hover{border-color:#cbd5e1; box-shadow:0 3px 10px rgba(15,23,42,.08);}
...
@media(max-width:640px){
 ...
 .stButton>button,.stButton>button{width:100%;}
}
</style>
...
if page == '🔄 리밸런싱 실행':
    ...
    fetch_clicked = pc2.button('🔄 가격 조회 실행', type='primary', width='stretch')
    ...
    if fetch_clicked:
        ...
        _prog.empty()
        # (스펙 기반) 트리거 판정용 최근 영업일 고점대비 하락률 — drawdown_* 규칙의 signal 파라미터에서 신호 티커·룩백을 읽는다
        trigger_dd = {}
        for code, spec in get_specs().items():
            rule = spec.get('rule', '')
            sig = (spec.get('params') or {}).get('signal')
            if rule not in ('drawdown_buy', 'drawdown_shift') or not sig:
                continue
            st_ticker = sig.get('ticker'); signal_market = sig.get('market', 'US')
            lookback = int(sig.get('lookback_days', 120))
            if not st_ticker or st_ticker == 'CASH':
                continue
            try:
                if signal_market == 'US':
                    trigger_dd[code] = fetch_yahoo_signal_drawdown(st_ticker, run_date.isoformat(), lookback, force_refresh=force_refresh_prices)
                else:
                    d = fetch_price_daily_recent(signal_market, source, st_ticker, run_date.isoformat(), lookback, force_refresh=force_refresh_prices)
                    trigger_dd[code] = drawdown_from_peak(d.sort_values('date')['close'].tolist()) if not d.empty else None
            except Exception as e:
                errors.append(f'{st_ticker}(트리거): {e}'); trigger_dd[code] = None
        ...
        if ok:
            ...
            if _fr_rows:
                with st.expander(f'ℹ️ 가격 신선도·출처 ({len(_fr_rows)}종목)', expanded=False):
                    st.dataframe(pd.DataFrame(_fr_rows), width='stretch', hide_index=True)
    ...
    plan_rows = []
    # 규칙 엔진 디스패치 — 스펙의 rule 이 RULE_REGISTRY 함수로 매핑된다 (if/elif 제거).
    # 스펙에 없는(구버전 데이터의) 전략은 정적 비중 복원으로 처리한다.
    _specs_now = get_specs()
    _engine_ctx = {'quarter_end': QUARTER_END, 'trigger_dd': trigger_dd}
    for code in ordered_strategy_codes(cfgs):
        sub_all = vdf[vdf['전략'] == code]
        if sub_all.empty:
            continue
        spec = _specs_now.get(code)
        if spec is not None:
            plan_rows.extend(se.apply_strategy(spec, vdf, _engine_ctx))
        else:
    ...
    # 전략별 계획 생성이 끝난 뒤 반드시 DataFrame으로 확정합니다.
    plan_df = pd.DataFrame(plan_rows)
    st.subheader('이번 달 상태 & 액션플랜')
    st.caption('전략 우선순위(스펙 display_order) 순으로 전략별 표를 보여줍니다. 매수는 빨간 볼드, 매도는 파란 볼드로 표시됩니다. 규칙·파라미터는 설정 페이지에서 수정할 수 있습니다.')
    if plan_df.empty or vdf.empty:
        ...
        for strat in order:
            ...
            st.dataframe(show.style.apply(color_action, axis=1), width='stretch', hide_index=True)
    ...
    st.markdown('#### 리밸런싱 기록 저장')
    st.caption('평소에는 오른쪽의 ⭐ 저장 + JSON 백업을 권장합니다. 같은 날짜로 다시 저장하면 기존 히스토리를 덮어씁니다.')
    _sv1, _sv2 = st.columns(2)
    with _sv1:
        if st.button('히스토리만 저장', key='save_history_only'):
            total_saved = save_history_snapshot(assets, run_date, plan_text=plan_text)
            st.success(f'히스토리에 저장했습니다. (총자산 {w(total_saved)})'); toast('히스토리에 저장했습니다.')
    with _sv2:
        _backup_payload = json.dumps(backup_with_snapshot(assets, run_date, plan_text), ensure_ascii=False, indent=2, default=str)
        def _save_before_download():
            total_saved = save_history_snapshot(assets, run_date, plan_text=plan_text)
            st.session_state['_save_backup_notice'] = f'히스토리 저장 + JSON 백업 완료 · 총자산 {w(total_saved)}'
        st.download_button(
            '⭐ 히스토리 저장 + JSON 백업',
            data=_backup_payload,
            file_name=f'rebalance-backup-{run_date.isoformat()}.json',
            mime='application/json',
            key='save_history_and_backup',
            on_click=_save_before_download,
            type='primary',
            width='stretch',
            help='한 번 클릭하면 현재 리밸런싱 히스토리를 저장하고 같은 상태의 JSON 백업 파일을 다운로드합니다.'
        )
    if st.session_state.get('_save_backup_notice'):
        st.success(st.session_state.pop('_save_backup_notice'))
    ...
    with st.expander('➕ 새 전략 추가'):
        st.caption('새 전략은 기본 규칙 static(목표비중 복원)으로 스펙 파일에 등록됩니다. 규칙·파라미터는 아래 "🧩 리밸런싱 규칙"에서 조정하세요.')
        ...
        if st.button('전략 추가'):
            ...
            if not code_clean:
                ...
                put_state('strategies', cfgs)
                se.upsert_spec_entry(SPEC_PATH, code_clean, rule='static', account=new_account.strip() or code_clean, description=new_desc.strip(), dynamic=new_dynamic)
                assets.loc[len(assets)] = {'id': str(len(assets) + 1), 'strategy': code_clean, 'ticker': 'CASH', 'name': '현금',
                                            ...
                                            'prices': [], 'signal_ticker': 'CASH', 'category': '현금'}
                st.session_state.assets = assets; put_state('assets', assets.to_dict('records'))
                st.success(f'{code_clean} 전략을 추가했습니다. 기본 규칙은 static(목표비중 복원)이며 아래에서 규칙·파라미터를 바꿀 수 있습니다.'); st.rerun()

    chosen = st.selectbox('전략 선택', codes, format_func=lambda x: f"{x} · {next((c.get('account', x) for c in cfgs if c.get('code') == x), x)}")
    ...
    with mgmt2:
        _ui_spec = get_specs().get(chosen)
        if _ui_spec is not None and _ui_spec.get('rule') != 'static':
            st.caption(f'{chosen}은(는) 전용 리밸런싱 규칙({_ui_spec.get("rule")})이 스펙에 정의되어 이름 변경이 불가합니다. 계좌 별명만 바꿔주세요.')
        else:
    ...
    with mgmt3:
        ...
        if st.button('🗑 전략 삭제', key=f'delbtn_{chosen}', disabled=not confirm_del):
            ...
            se.remove_spec_entry(SPEC_PATH, chosen)
            st.session_state.assets = assets2
            st.success(f'{chosen} 전략·소속 종목·스펙을 모두 삭제했습니다.'); st.rerun()

    with st.expander('🧩 전략 규칙 쉽게 수정하기', expanded=False):
        st.caption('전략의 숫자·기준 티커·기간은 여기서 바꾸면 됩니다. Python 코드를 수정할 필요가 없습니다.')
        _ui_specs = get_specs()
        _sp_ui = _ui_specs.get(chosen) or {'rule': 'static', 'params': {}, 'description': '', 'dynamic': False, 'account': chosen}
        _rules_ui = list(se.RULE_REGISTRY.keys())
        _rule_ui = _sp_ui.get('rule') if _sp_ui.get('rule') in _rules_ui else 'static'
        _sel_rule = st.selectbox(
            '이 전략은 어떻게 리밸런싱할까요?', _rules_ui,
            index=_rules_ui.index(_rule_ui), key=f'specrule_{chosen}',
        ...
        _params_to_save = _friendly_params
        _json_error = None
        if _advanced:
            with st.expander('JSON 직접 편집', expanded=True):
                _params_ui = st.text_area('파라미터 JSON', value=json.dumps(_friendly_params, ensure_ascii=False, indent=2), height=180, key=f'specparams_{chosen}')
                try:
                    _parsed = json.loads(_params_ui) if _params_ui.strip() else {}
                    if not isinstance(_parsed, dict):
                        raise ValueError('JSON 객체여야 합니다.')
                    _params_to_save = _parsed
        ...
        if st.button('전략 규칙 저장', key=f'specsave_{chosen}', type='primary', disabled=bool(_json_error)):
            se.upsert_spec_entry(SPEC_PATH, chosen, rule=_sel_rule, params=_params_to_save,
                                 account=_sp_ui.get('account', chosen), description=_desc_ui,
                                 dynamic=_sp_ui.get('dynamic', False), display_order=_sp_ui.get('display_order', 99))
            _cfgs3 = get_strategies()
            for _c in _cfgs3:
                if _c['code'] == chosen:
                    _c['description'] = _desc_ui
    ...
    if not subset.empty:
        ...
        if st.button('선택 종목 삭제', key=f'del_btn_{chosen}', disabled=not del_pick):
            ...
            assets2 = assets[~((assets['strategy'].eq(chosen)) & (assets['ticker'].isin(del_tickers)))].copy()
            st.session_state.assets = assets2; put_state('assets', assets2.to_dict('records'))
            st.success(f'{len(del_tickers)}개 종목을 삭제했습니다.'); toast('종목을 삭제했습니다.'); st.rerun()
    else:
    ...
    hist_date_text = st.text_input('저장할 날짜 (YYYY-MM-DD)', date.today().isoformat(), key='hist_save_date_text')
    hist_date = parse_ui_date(hist_date_text, '저장할 날짜')
    _past_date = hist_date is not None and hist_date < date.today()
    if _past_date:
        st.caption('과거 날짜로 저장하면 해당 날짜의 종가를 캐시에서 재사용하고(없으면 다시 조회) 그 시점 평가액으로 기록합니다. 조회할 수 없는 종목은 현재 종가로 저장됩니다.')
    if st.button('입력한 날짜로 전체 스냅샷을 히스토리에 저장', type='primary', key='save_snapshot_btn', disabled=hist_date is None):
        if _past_date:
    ...
    if not h:
        ...
        if view.empty:
            ...
            with tab1:
                ...
                for _ri, _r in _ordered_view.iterrows():
                    _ds = str(_r['date'])
                    _hc1, _hc2, _hc3, _hc4 = st.columns([1.15, 1.25, 2.7, .7])
                    _hc1.markdown(f'**{_ds}**')
                    _hc2.markdown(f'**{w(n(_r.get("total")))}**')
                    _hc3.caption(str(_r.get('plan') or '저장된 액션플랜 메모 없음')[:110])
                    if _hc4.button('삭제', key=f'hist_row_del_{_ds}', help=f'{_ds} 히스토리만 삭제합니다.'):
                        delete_history_snapshot(_ds)
                        st.success(f'{_ds} 히스토리를 삭제했습니다. 같은 날짜로 다시 저장할 수 있습니다.')
                        st.rerun()
            ...
            with tab3:
                ...
                if not by_cat.empty:
                    ...
                    st.dataframe(show.rename(columns={'date':'날짜'}),width='stretch',hide_index=True)
                ...
                if rec:
                    ...
                    if comp:
                        ...
                        if MOBILE:
                            ...
                            show=cdf.copy(); show['현재금액']=show['현재금액'].map(w); show['현재비중']=show['현재비중'].map(lambda x:f'{x:.1f}%'); show['목표비중']=show['목표비중'].map(lambda x:f'{x:.1f}%'); st.dataframe(show,width='stretch',hide_index=True)
                    ...
                    if exec_recs:
                        ...
                        st.dataframe(edf[['strategy','ETF','실행','planned','actual','차이']].rename(columns={'strategy':'전략','planned':'계획','actual':'실제'}),width='stretch',hide_index=True)
                    ...
                    st.caption('히스토리 삭제는 요약 탭의 각 행 오른쪽 `삭제` 버튼에서 할 수 있습니다.')
    ...
    with bc1:
        ...
        download_link('JSON 백업 다운로드',
                      json.dumps(export_backup_dict(), ensure_ascii=False, indent=2, default=str),
                      f'portfolio-backup-{date.today().isoformat()}.json', 'application/json')
        if h:
            download_link('CSV 히스토리(요약)',
                          pd.DataFrame(h).drop(columns=['composition', 'by_strategy', 'by_category'], errors='ignore').to_csv(index=False),
                          'rebalance-history.csv', 'text/csv')
        backup_dir = Path.home() / '.asset_allocation_app' / 'backups'
    ...
    with bc2:
        ...
        st.caption('JSON은 직접 붙여넣습니다. Streamlit FileUploader JS chunk 의존성을 제거했습니다.')
        restore_text = st.text_area('JSON 내용 붙여넣기', height=220, key='restore_json_text',
                                    placeholder='{\n  "assets": [...],\n  "strategies": [...]\n}')
    ...
    with st.expander('🛠️ 고급 · 문제 해결', expanded=False):
        ...
        with cc1:
            if st.button('🗑️ 가격 캐시 전체 삭제', width='stretch'):
                cache_clear_prices()
                st.success('SQLite 가격 캐시와 Yahoo 런타임 캐시를 함께 삭제했습니다.')
        with cc2:
            if st.button('♻️ 가격 조회 캐시만 초기화', width='stretch'):
    ...
    cd_text = st.text_input('거래일 (YYYY-MM-DD)', date.today().isoformat(), key='cd_text'); cd = parse_ui_date(cd_text, '거래일'); ca = st.number_input('금액(입금 + / 출금 -)', step=100000.0, key='ca')
    cstrat = st.selectbox('전략(계좌)', cf_codes, key='cstrat'); cm = st.text_input('메모', key='cm')
    if st.button('입출금 저장', disabled=cd is None):
    ...
    if not cf_show.empty:
        ...
        cf_edit = st.data_editor(
            cf_show, num_rows='dynamic', width='stretch', hide_index=True,
            column_config={
    ...
    with st.expander('벌크 입력 · 정기납입 자동 생성', expanded=False):
        ...
        csv_text = st.text_area('입출금 CSV 내용 붙여넣기', height=140, key='cf_csv_text',
                                placeholder='date,amount,memo,strategy\n2026-09-01,1000000,월납입,ISA')
        if csv_text.strip():
            try:
        ...
        _cf_start_text = rc1.text_input('시작일 (YYYY-MM-DD)', date.today().isoformat(), key='cf_start_text')
        _cf_start = parse_ui_date(_cf_start_text, '정기납입 시작일')
        ...
        if st.button('정기납입 기록 생성(추가)', key='cf_recur_add', disabled=_cf_start is None):
            cur = get_state('cashflows')
