...
if page == '🔄 리밸런싱 실행':
    ...
    with st.expander('🧩 리밸런싱 규칙 (스펙) — 코드 수정 없이 조정', expanded=False):
        st.caption('규칙은 함수가 아니라 스펙 파일(strategy_spec.json)의 rule 필드로 선택됩니다. 파라미터를 바꾸고 저장하면 다음 리밸런싱부터 적용됩니다.')
        _ui_specs = get_specs()
        _sp_ui = _ui_specs.get(chosen)
        if _sp_ui is None:
            st.caption(f'{chosen}은(는) 아직 스펙에 없습니다. 규칙을 지정하면 스펙에 등록됩니다.')
            _sp_ui = {'rule': 'static', 'params': {}, 'description': '', 'dynamic': False, 'account': chosen}
        _rules_ui = list(se.RULE_REGISTRY.keys())
        _rule_ui = _sp_ui.get('rule') if _sp_ui.get('rule') in _rules_ui else 'static'
        _sel_rule = st.selectbox('적용 규칙', _rules_ui, index=_rules_ui.index(_rule_ui), key=f'specrule_{chosen}',
                                 format_func=lambda x: f'{x} — {se.RULE_DESC.get(x, "")}')
        _params_ui = st.text_area('파라미터 (JSON 객체)', value=json.dumps(_sp_ui.get('params') or {}, ensure_ascii=False, indent=2), height=140, key=f'specparams_{chosen}',
                                  help='예: {"threshold": -0.12, "buy_fraction": 0.5} — 규칙마다 사용하는 키가 다릅니다.')
        _desc_ui = st.text_input('전략 설명', value=_sp_ui.get('description', ''), key=f'specdesc_{chosen}')
        if st.button('규칙/파라미터 저장', key=f'specsave_{chosen}'):
            try:
                _parsed = json.loads(_params_ui) if _params_ui.strip() else {}
                if not isinstance(_parsed, dict):
                    raise ValueError('JSON 객체여야 합니다 (예: {"threshold": -0.12})')
            except Exception as _e:
                st.error(f'파라미터 JSON 오류: {_e}')
            else:
                se.upsert_spec_entry(SPEC_PATH, chosen, rule=_sel_rule, params=_parsed,
                ...
                st.success(f'{chosen} 규칙·파라미터를 저장했습니다. (rule: {_sel_rule})'); st.rerun()
